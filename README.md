...
| **Dynamic Graph v2.0** | CN/EN-aware auto-update ?中文期刊自动填入 `authors_zh`，新作?期刊自动注册，中英文双语去重 | `src/graph_updater.py` |
| **CN/EN Literature Rule** | 中文期刊走中文署名（杨计平），英文走英文名（Yang Jiping）；论文防双版本重复 | `project memory (high)` |
| **MCP Timeout Protection** | 15 ?threading 超时防止 MCP 子进程永久阻?| `src/mcp_client.py` |
| **Chinese Academic Search Skill** | 覆盖 8 种中文期刊的专用搜索策略 | `skills/chinese-academic-search.md` |

### Meso-Cosmos Architecture

```
┌─────────────────────────────────────────────────────??             Macro-cosmos (BDI 意图?               ?? CognitiveAgent · WorldModel · Belief/Desire/Intention ?└──────────────────────┬──────────────────────────────?                       ?┌──────────────────────▼──────────────────────────────??             Meso-cosmos (协调?                     ?? MesoAgent.search(species_id)                       ??                                                    ?? Pipeline: BDI预测 ?模式选择(穷举/分类/轻量)          ??         ?执行分发 ?图谱更新 ?CN/EN规则            ??                                                    ?? Components: WorldModel + SearchRuleEngine          ??             + MemorySystem + GraphUpdater          ?└──────────────────────┬──────────────────────────────?                       ?┌──────────────────────▼──────────────────────────────??             Micro-cosmos (执行?                    ?? PubMed E-utilities · Crossref · OpenAlex · MCP      ?? 11 search phases · 5 engines · Authority scoring    ?└─────────────────────────────────────────────────────?```
...
─────────────────────────────────────
Exact name      ─?Variant spell   ─?Author network  ─┼─?Ochetobius elongatus (?
Citation graph  ─?Journal context ─?Chinese name    ─?```
...
| 🎛?MPC World Model | ?| Search cost optimization |
...
?  ├── agent_judge.py            ?⚖ LLM-as-Judge 4-dimension evaluation
...
| **Benchmark** | Only verified on ?(12 papers) | Unknown recall on other species |
...
- pip install ?`python -m cognitive_search.search "?`
...
>
> We say: You cannot analyze today is ecological data with last month is code.
